...
// const API_BASE_URL = "https://localhost:3000" || process.env.RENDER_URL;
...
const state = {
  ...
  connectToGamerooms(roomId) {
    ...
    onValue(gameRoomsRef, (snapshot) => {
      ...
      // de la app con el score actualizado y se llama a la
      // función setState para actualizar la interfaz de usuario con la nueva
      // información.
      fireBaseScorePromise.then((scoreData) => {
        currentState.roomScore = scoreData;
        this.setState(currentState);
  ...
  reconnectToGamerooms(roomId) {
    //rtdbGameroomId: es el parámetro de la sala específica a la que se hace referencia
    console.log("soy state.recoconnectToGameRooms con RTDBgameroomId", roomId);
